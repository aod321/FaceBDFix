...
if __name__ == '__main__':
    model = resnet_fpn_backbone(backbone_name='resnet18', pretrained=True)
    print(model)
    input_test = torch.randn(1, 3, 512, 512)
    out_test = model(input_test)
    print(out_test[0]['2'].shape)
    print(out_test[0]['3'].shape)
    print(out_test[0]['4'].shape)
    print(out_test[0]['5'].shape)

    print(out_test[1]['2'].shape)
    print(out_test[1]['3'].shape)
    print(out_test[1]['4'].shape)
    print(out_test[1]['5'].shape)
    print(out_test[1]['pool'].shape)
