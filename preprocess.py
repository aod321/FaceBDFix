...
class Resize(transforms.Resize):
    ...
    def __call__(self, sample):
        ...
        resized_labels = [TF.resize(labels[r], self.size, InterpolationMode.NEAREST)
...
class Stage1_ToTensor(transforms.ToTensor):
    """Convert a ``PIL Image`` or ``numpy.ndarray`` to tensor.
...
class Skin_ToTensor(transforms.ToTensor):
    ...
    def __call__(self, sample):
        ...
        sample.update({'image': image, 'labels': labels, 'name': sample['name']})
...
class RandomAffine(transforms.RandomAffine):
    ...
    def __call__(self, sample):
        ...
        img = TF.affine(img, *ret, interpolation=self.resample, fill=self.fill)
        labels = [TF.affine(labels[r], *ret, interpolation=self.resample, fill=self.fill)
                  for r in range(len(labels))]
...
class Stage2_GaussianNoise(object):
    def __call__(self, sample):
        ...
        sample.update({'image': parts, 'labels': sample['labels']})
        return sample
...
class OldStage2Resize(transforms.Resize):
    ...
    def __call__(self, sample):
        ...
        sample.update({'image': resized_image,
                  'labels': labels
                  })
