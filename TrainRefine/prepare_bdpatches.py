...
plt_show = False
parts_name = ['skin', 'lbrow', 'rbrow', 'leye', 'reye', 'nose', 'mouth', 'hair']
size_set = [32, 16, 16, 16, 16, 16, 16, 64]
# size_set = [32, 32, 32, 32, 32, 32, 32, 32]
# prepare parts bd
cropped_parts_bd_out_path = "/bigdata/yinzi/patchset_test/parts_skin_hair"
for x in parts_name:
...
for x in ['train', 'val', 'test']:
    for batch in tqdm(dataloader[x]):
        ...
        # label Shape [1, 11, 512, 512]
        label = torch.cat([label[:, 1:7], label[:, 7:10].sum(dim=1, keepdim=True), label[:, 10:11]], dim=1)
        for i in range(8):
