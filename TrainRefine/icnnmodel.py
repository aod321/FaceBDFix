...
class iCNN_Node(nn.Module):
    def __init__(self, in_channels=3, out_channels=32,
                 inter_in_channels=3, inter_out_channels=32,
                 output_in_channels=3, output_out_channels=32,
                 kernel_size=3, stride=1,
                 padding=1,
                 interpol_size=3, interpol_mode='nearest',
                 pool_size=3, pool_stride=1,
                 conv_node_batchnorm_C=3, inter_conv_node_batchnorm_C=3,
                 output_conv_node_batchnorm_C=3):
        ...
        self.inter_in_channels = inter_in_channels
        self.inter_out_channels = inter_out_channels
        # Output Intergration
        self.output_in_channels = output_in_channels
        self.output_out_channels = output_out_channels
        ...
        self.conv_node_batchnorm_C = conv_node_batchnorm_C
        self.inter_conv_node_batchnorm_C = inter_conv_node_batchnorm_C
        self.output_conv_node_batchnorm_C = output_conv_node_batchnorm_C
        ...
        self.inter_conv_node = nn.Conv2d(in_channels=self.inter_in_channels,
                                         out_channels=self.inter_out_channels, kernel_size=self.
                                         kernel_size, stride=self.stride, padding=self.padding)
    ...
    def set_conv_bachnorm(self, c=3, group=None):
        self.conv_node_batchnorm_C = c
        if group is not None:
            self.conv_node_batchnorm = nn.GroupNorm(num_groups=group,
                                                    num_channels=self.conv_node_batchnorm_C)
        else:
            self.conv_node_batchnorm = nn.BatchNorm2d(num_features=self.conv_node_batchnorm_C)

    def set_inter_conv_node_batchnorm(self, c=3, group=None):
        self.inter_conv_node_batchnorm_C = c
        if group is not None:
            self.inter_conv_node_batchnorm = nn.GroupNorm(num_groups=group,
                                                          num_channels=self.inter_conv_node_batchnorm_C
                                                          )
        else:
            self.inter_conv_node_batchnorm = nn.BatchNorm2d(num_groups=group,
                                                            num_channels=self.inter_conv_node_batchnorm_C)

    def set_output_conv_node_batchnorm(self, c=3, group=None):
        self.inter_conv_node_batchnorm_C = c
        if group is not None:
            self.output_conv_node_batchnorm = nn.GroupNorm(num_groups=group,
                                                           num_channels=self.inter_conv_node_batchnorm_C
                                                           )
        else:
            self.output_conv_node_batchnorm = nn.BatchNorm2d(self.output_conv_node_batchnorm_C)
    ...
    def forward(self, x, feature_from_pre, feature_from_post):
        ...
        if feature_from_pre is not None and feature_from_post is not None:
            ...
            x = torch.cat([feature_from_pre, x, feature_from_post], dim=1)
        else:
            if feature_from_pre is None and feature_from_post is None:
                ...
                if feature_from_pre is None:
                    feature_from_post = self.upsample(feature_from_post)
                    x = torch.cat([x, feature_from_post], dim=1)

                else:  # feature_from_post is None:
                    feature_from_pre = self.downsample(feature_from_pre)
                    x = torch.cat([feature_from_pre, x], dim=1)

        # filters = (torch.ones((x.shape[1], x_out.shape[1], 3, 3)))
        x = F.relu(self.inter_conv_node_batchnorm(self.inter_conv_node(input=x)), inplace=True)
        x = F.relu(self.conv_node_batchnorm(self.conv_node(x)), inplace=True)

        return x


class iCNN_Cell(nn.Module):
    ...
    def __init__(self, recurrent_number, in_channels, out_channels, kernel_size, stride,
        ...
        for i in range(self.recurrent_number):
            ...
            if Norm_layer == 'BN':
                self.iCNN_Nodelist[i].set_conv_bachnorm(c=self.out_channels[i])
            elif Norm_layer == 'GN':
                if self.out_channels[i] % 3 == 0:
                    group = 3
                else:
                    group = 4
                self.iCNN_Nodelist[i].set_conv_bachnorm(c=self.out_channels[i], group=group)

            self.iCNN_Nodelist[i].set_inetr_Conv(inter_in[i], self.in_channels[i])

            if Norm_layer == 'BN':
                self.iCNN_Nodelist[i].set_inter_conv_node_batchnorm(c=self.in_channels[i])
            elif Norm_layer == 'GN':
                if self.in_channels[i] % 3 == 0:
                    group = 3
                else:
                    group = 4
                self.iCNN_Nodelist[i].set_inter_conv_node_batchnorm(c=self.in_channels[i], group=group)

            self.iCNN_Nodelist[i].set_output_Conv(output_in[i], self.in_channels[i])
            if Norm_layer == 'BN':
                self.iCNN_Nodelist[i].set_output_conv_node_batchnorm(c=self.in_channels[i])
            elif Norm_layer == 'GN':
                if self.in_channels[i] % 3 == 0:
                    group = 3
                else:
                    group = 4
                self.iCNN_Nodelist[i].set_output_conv_node_batchnorm(c=self.in_channels[i], group=group)
    ...
    def get_output_integration(self):
        ...
        for i in range(self.recurrent_number - 1, 0, -1):
            ...
            # cat_feature = torch.cat([self.interlink_features[i - 1],
            #                          self.iCNN_Nodelist[i - 1].upsample(self.interlink_features[i])],
            #                         dim=1)

            # Use conv2d to compute  feature_pre += feature_to_pre
            self.interlink_features[i - 1] = torch.tanh(
                self.iCNN_Nodelist[i - 1].output_conv_node_batchnorm(
                    self.iCNN_Nodelist[i - 1].output_conv_node(
                        torch.cat([self.interlink_features[i - 1],
                                   self.iCNN_Nodelist[i - 1].upsample(self.interlink_features[i])],
                                  dim=1)
                    )
                )
...
class ICNNBackBone(torch.nn.Module):
    def __init__(self, in_channels=3):

        super(ICNNBackBone, self).__init__()
        ...
        self.input_bnm = nn.ModuleList([])
        for i in range(self.recurrent_number):
            if self.first_channels_size[i] % 3 == 0:
                group = 3
            else:
                group = 4
            self.input_bnm.append(nn.GroupNorm(num_groups=group, num_channels=self.first_channels_size[i]))
        # interlink_layer0 interlink_layer1 interlink_layer2
        # self.fixed_theta = self.fixed_theta.type_as(x)
...
class ICNNSegModel(torch.nn.Module):

    def __init__(self,  in_channels=3, out_channels=9):

        super(ICNNSegModel, self).__init__()

        self.icnn_backbone = ICNNBackBone(in_channels=in_channels)

        self.lable_channel_size = out_channels
        # Parameters initiation
        ...
        self.last_conv1 = nn.Conv2d(in_channels=self.first_channels_size[0],
                                    ...
                                    padding=self.kernel_size // 2)
        self.last_con1_bnm = nn.GroupNorm(num_groups=2, num_channels=2 * self.lable_channel_size + 8)
        ...
        self.last_conv2 = nn.Conv2d(in_channels=2 * self.lable_channel_size + 8,
                                    ...
                                    kernel_size=self.kernel_size, stride=1,
                                    padding=self.kernel_size // 2)
        self.last_con2_bnm = nn.GroupNorm(2, self.lable_channel_size)
        ...
        self.last_conv3_bnm = nn.GroupNorm(2, self.lable_channel_size)

    def forward(self, x):
        final_output = self.icnn_backbone(x)
        # Final Output
...
class ICNNSegModel_old(torch.nn.Module):

    def __init__(self, in_channels=3, out_channels=9, interlink_layer_number=3):

        super(ICNNSegModel_old, self).__init__()
        ...
        self.in_channels = in_channels
        self.interlink_layer_number = interlink_layer_number
        self.recurrent_number = 4
        self.kernel_size = 5
        self.last_kernel_size = out_channels
